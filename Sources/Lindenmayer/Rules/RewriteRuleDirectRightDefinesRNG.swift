...
///
/// - ``init(directType:rightType:parameters:prng:where:produces:)``
///
...
///
public struct RewriteRuleDirectRightDefinesRNG<DC, RC, PType, PRNG>: Rule where DC: Module, RC: Module, PRNG: SeededRandomNumberGenerator, PType: Sendable {
    ...
    public typealias CombinationMatchProducesList = @Sendable (DC, RC, PType, RNGWrapper<PRNG>) async -> [any Module]
    ...
    public func produce(_ matchSet: ModuleSet) async -> [any Module] {
        ...
        return await produceClosure(directInstance, rightInstance, parameters, prng)
    }
}
