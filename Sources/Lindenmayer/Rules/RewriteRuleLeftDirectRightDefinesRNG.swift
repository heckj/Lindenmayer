...
public struct RewriteRuleLeftDirectRightDefinesRNG<LC, DC, RC, PType, PRNG>: Rule where LC: Module, DC: Module, RC: Module, PRNG: SeededRandomNumberGenerator, PType: Sendable {
    ...
    public typealias Eval = @Sendable (LC, DC, RC, PType) -> Bool
    /// The signature of the produce closure that provides a module and expects a sequence of modules.
    public typealias CombinationMatchProducesList = @Sendable (LC, DC, RC, PType, RNGWrapper<PRNG>) async -> [any Module]
    ...
    public func produce(_ matchSet: ModuleSet) async -> [any Module] {
        ...
        return await produceClosure(leftInstance, directInstance, rightInstance, parameters, prng)
    }
}
...
extension RewriteRuleLeftDirectRightDefinesRNG: CustomStringConvertible {
    ...
    public var description: String {
        "Rule(left,direct,right)(\(String(describing: matchingTypes)) w/ parameters: \(String(describing: parameters)) and rng"
    }
}
