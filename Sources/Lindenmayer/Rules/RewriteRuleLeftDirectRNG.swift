//
//  RewriteRuleLeftDirectRNG.swift
//
//
...
///
/// - ``init(leftType:directType:prng:where:produces:)``
///
...
public struct RewriteRuleLeftDirectRNG<LC, DC, PRNG>: Rule where LC: Module, DC: Module, PRNG: SeededRandomNumberGenerator {
    ...
    public var parametricEval: (@Sendable (LC, DC) -> Bool)?

    /// The signature of the produce closure that provides a module and expects a sequence of modules.
    public typealias CombinationMatchProducesList = @Sendable (LC, DC, RNGWrapper<PRNG>) async -> [any Module]
    ...
    public func produce(_ matchSet: ModuleSet) async -> [any Module] {
        ...
        return await produceClosure(leftInstance, directInstance, prng)
    }
}
