//
//  RewriteRuleLeftDirectDefinesRNG.swift
//
//
...
///
/// - ``init(leftType:directType:parameters:prng:where:produces:)``
///
...
public struct RewriteRuleLeftDirectDefinesRNG<LC, DC, PType, PRNG>: Rule where LC: Module, DC: Module, PRNG: SeededRandomNumberGenerator, PType: Sendable {
    ...
    public func produce(_ matchSet: ModuleSet) async -> [Module] {
        ...
        return await produceClosure(leftInstance, directInstance, parameters, prng)
    }
}

extension RewriteRuleLeftDirectDefinesRNG: CustomStringConvertible {
    /// A description of the rule that details what it matches
    public var description: String {
        "Rule(left,direct)(\(String(describing: matchingTypes)) w/ parameters: \(String(describing: parameters)) and rng"
    }
}
