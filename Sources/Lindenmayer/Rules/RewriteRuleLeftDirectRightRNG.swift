...
///
/// - ``init(leftType:directType:rightType:prng:where:produces:)``
///
...
public struct RewriteRuleLeftDirectRightRNG<LC, DC, RC, PRNG>: Rule where LC: Module, DC: Module, RC: Module, PRNG: SeededRandomNumberGenerator {
    ...
    public typealias CombinationMatchProducesList = @Sendable (LC, DC, RC, RNGWrapper<PRNG>) async -> [any Module]
    ...
    public func produce(_ matchSet: ModuleSet) async -> [any Module] {
        ...
        return await produceClosure(leftInstance, directInstance, rightInstance, prng)
    }
}
...
extension RewriteRuleLeftDirectRightRNG: CustomStringConvertible {
    ...
    public var description: String {
        "Rule(left,direct,right)(\(String(describing: matchingTypes)) w/ rng"
    }
}
