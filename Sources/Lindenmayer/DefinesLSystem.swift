...
public struct DefinesLSystem<PType, PRNG>: LSystem where PRNG: RandomNumberGenerator {
    ...
    public func updatedLSystem(with state: [Module]) -> LSystem {
        return DefinesLSystem<PType, PRNG>(axiom: state, parameters: parameters, prng: prng, rules: rules)
    }
    ...
    public func rewrite(_ direct: Module.Type, _ singleModuleProduce: @escaping (Module, PType, Chaos<PRNG>) throws -> [Module]) -> Self {
        let newRule = DefinesRule(direct, params: self.parameters, prng: self.prng, singleModuleProduce)
        var newRuleSet: [Rule] = self.rules
        newRuleSet.append(contentsOf: [newRule])
        return DefinesLSystem(axiom: self.state, parameters: self.parameters, prng: self.prng, rules: newRuleSet)
    }
    ...
    /// - Returns: A new L-System with the additional rule added.
    public func rewrite(_ left: Module.Type?, _ direct: Module.Type, _ right: Module.Type?, _ multiModuleProduce: @escaping (Module?, Module, Module?, PType, Chaos<PRNG>) throws -> [Module]) -> Self {
        let newRule = DefinesRule(left, direct, right, params: self.parameters, prng: self.prng, multiModuleProduce)
        var newRuleSet: [Rule] = self.rules
        newRuleSet.append(contentsOf: [newRule])
        return DefinesLSystem(axiom: self.state, parameters: self.parameters, prng: self.prng, rules: newRuleSet)
    }
